...
def main():
    """
    CLI entry point for running the imputation training pipeline.
    Reads input parameters from a YAML configuration file.
    """
    # Set up argument parser for CLI
    parser = argparse.ArgumentParser(description="Run the imputation training pipeline.")
    ...
    if config_path!="/price_imputation_and_forecasting/config/birnn_gru_config.yaml":
        ...
        price_imputer_forecaster = price_imputation_and_forecasting.PriceImputationForecasting()
    ...
    try:
        ...
        print(f"Imputation training completed for category '{category}'.")
        print("Evaluation metrics:")
        for metric, value in evaluation_metrics.items():
            print(f"{metric}: {value}")

    except Exception as e:
        print(f"Error running imputation training pipeline: {e}")

...
def train_lstm(
    ...
    **kwargs
    ...
    """
    Build, train, and return a LSTM model for price forecasting.
    ...
    # Reshape inputs
    # ---------------------------
    # Reshape inputs for LSTM → expects 3D (n, timesteps, features)
    ...
    # First LSTM → returns sequence for stacking
    x = LSTM(lstm_units, return_sequences=True, dropout=dropout_rate, recurrent_dropout=dropout_rate)(sequence_input)

    # Second LSTM → compress sequence into final hidden state (a smaller representation)
    x = LSTM(lstm_units // 2, return_sequences=False, dropout=dropout_rate, recurrent_dropout=dropout_rate)(x)

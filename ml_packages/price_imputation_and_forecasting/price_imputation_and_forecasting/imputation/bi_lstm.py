...
def train_bi_lstm(
    ...
    **kwargs
    ...
    """
    Build, train, and save a Bi-LSTM model for price imputation.
    ...
    - imputation_model_params (dict): Dictionary of hyperparameters passed from config.
        - seq_units (int): Number of BiLSTM units.
    ...
    Returns:
    - model (tf.keras.Model): Trained Bi-LSTM model.
    ...
    # ----------------------------------------------------
    seq_units = kwargs.get("seq_units", 128)  # per-direction units for each LSTM
    dense_units = kwargs.get("dense_units", 128)  # dense units used in static pathway and decoder
    dropout_rate = kwargs.get("dropout_rate", 0.2)
    recurrent_dropout = kwargs.get("recurrent_dropout", 0.1)  # regularize LSTM recurrent connections
    ...
    # First BiLSTM block (returns full sequence)
    x = Bidirectional(LSTM(seq_units, return_sequences=True, activation="tanh", recurrent_dropout=recurrent_dropout), name="bidir_lstm_1")(sequence_input)
    x = Dropout(dropout_rate, name="dropout_lstm_1")(x)  # Regularize the sequence encoding

    # Second BiLSTM block (returns full sequence)
    x2 = Bidirectional(LSTM(seq_units, return_sequences=True, activation="tanh", recurrent_dropout=recurrent_dropout), name="bidir_lstm_2")(x)
    x2 = Dropout(dropout_rate, name="dropout_lstm_2")(x2)
    ...
    # Expand static features across timesteps, so we can concat them to LSTM outputs
    # RepeatVector creates a (batch, seq_len, dense_units) tensor by copying the static vector T times
    # This allows concatenation with the LSTM outputs (batch, T, hidden_dim) to create (batch, T, hidden_dim + D)
